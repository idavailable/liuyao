...
- **夜子时流派显式声明**：`LY.nightZiMode`（默认 `'day'`：23 点归当日，主流派；`'next'`：归次日）
...
- **规则覆盖**：月建档（旺相休囚死）、真假月破、日辰生克拱扶、冲散/暗动/日破、合起合绊、静空/动空/真空（旬空+月破）、回头生克冲合、进神退神、三合局、相刑相害、飞伏生克、用神两现取舍、伏神引拔
- **质量保障**：`test-core.js` 37 项基础自检 + `test-rules.js` 49 项规则链单测 + `test-diff.mjs` 三库交叉差分（见下）
...
已达成：**104,716 个字段三方零差异、零流派分歧**；节气表 1900–2100 全量 2412 项与 lunar-javascript 对齐（误差 = 0）。差分曾抓出真实存量 bug：64 卦名表中「山火贲」曾被误标（旅卦覆卦混淆），现已修复并回归。
...
| `test-rules.js` | 断卦规则链 49 项单测（暗动闭环、进退神、三合、夜子时、交节边界等） |
| `test-diff.mjs` | 三库交叉差分（4096 组合 + 历法时点 + 节气全量，10 万+字段） |
