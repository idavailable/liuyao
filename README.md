...
- Karplus-Strong 拨弦物理建模（噪声激励 + 延时反馈低通），音色近似丝弦
- D 宫五声音阶随机漫步成句，泛音/双音点缀，句间留白，卷积混响
...
- **卦例库**：D1 数据库存储历史卦例（含各模型对话记录）；浏览开放，**装入需口令登录**（LIB_CODE）
- **生成式琴音**：右下角「琴」按钮，纯 Web Audio 合成古琴背景音（零音频文件）
